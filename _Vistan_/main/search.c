...
void pid_test()
{
	...
	VFDPrintf("PID %f",_IQ17toF(g_q17user_vel));
	DELAY_US(1000000);
	VFDPrintf("        ");
	DELAY_US(500000);
	...
	/*
	pm->q28kp = _IQ28( 0.37 );	//0.6	 // 0.8
	pm->q28ki = _IQ28( 0 );
	pm->q28kd = _IQ28( 0.62 );  //0.65	//0.85 
	*/
				if(pid_flag){
			
					if(Up_SW){
					
						g_rm.q28kp += _IQ28(0.05);
						DELAY_US(150000);
					}
					else if(Down_SW){
					...
					VFDPrintf("%lf",_IQ28toF(g_rm.q28kp));
				}
				else{	
					...
					VFDPrintf("%lf",_IQ28toF(g_rm.q28kd));
				}
			#endif	
		
		//TxPrintf(" %lf, %lf\n",_IQ17toF(_IQmpy(g_rm.q17next_vel , g_q17right_handle  )), _IQ17toF(g_rm.q17cur_vel_avr));
